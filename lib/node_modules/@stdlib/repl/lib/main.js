...
var previewCompleterFactory = require('./preview_completer.js');
...
function REPL( options ) {
	...
	previewCompleter = previewCompleterFactory(this);

	this._rli.on( 'close', onClose );
	this._rli.on( 'SIGINT', onSIGINT );
	this._rli.on( 'line', onLine );

	// Add an event listener for any key press on the readline input

	// Handle drawing the gray preview
	this._rli.input.on( 'keypress', previewCompleter.onKeypress );
	...
	// Add an event listener that fires before the keypress is

	// Processed by the readline input. Handle special cases

	// Related to preview, such as when users presses right to

	// Fill in the preview.
	ttyWrite = self._rli._ttyWrite.bind( self._rli );

	self._rli._ttyWrite = function beforeKeypress(d, key) {
		previewCompleter.beforeKeypress(d, key);
		ttyWrite(d, key);
	};
